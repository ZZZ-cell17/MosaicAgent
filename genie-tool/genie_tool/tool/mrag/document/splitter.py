...
url_match_pattern = "(https?://[A-Za-z0-9{}]+)".format(
    re.escape(url_allowed_chars + url_reserve_chars + url_unsafe_chars))
...
class SplitSentence(object):
    def __init__(self):
        ...
        separators_all = "{}|##|###|(\n\n)|(\r\n)|{}".format(url_match_pattern, "|".join(
            ["({})".format(re.escape(char)) for char in self.separators]))
...
class DefaultDocumentSplitter(BaseDocumentSplitter):
    def __init__(self, chunk_size: int = None, chunk_overlap: int = None):
        self._chunk_size = chunk_size if chunk_size is not None else int(os.getenv("CHUNK_SIZE", 500))
        self._chunk_overlap = (
            chunk_overlap if chunk_overlap is not None else int(os.getenv("CHUNK_OVERLAP", 100))
        )

    def split(self, text: str):
        chunks = text_split_to_chunks(
        ...
        bounded_chunks = []
        overlap = min(self._chunk_overlap, max(0, self._chunk_size - 1))
        step = self._chunk_size - overlap
        for chunk in chunks:
            if len(chunk) <= self._chunk_size:
                bounded_chunks.append(chunk)
                continue
            for start in range(0, len(chunk), step):
                bounded_chunks.append(chunk[start:start + self._chunk_size])
                if start + self._chunk_size >= len(chunk):
                    break
    ...
    def split(self, text: str):
        ...
        for doc in docs:
            ...
            if len(content) > self._chunk_size:
                ...
                for chunk_text in text_splitter.split(doc.page_content):
                    merged_chunks.append({
                        'content': chunk_text,

                        ...
                        for drawing in run.element.findall(
                            ...
                            for blip in drawing.findall(
                                ...
                                if embed_id:
                                    ...
                                    image_suffix = suffix_by_content_type.get(
                                        image_part.content_type,
                                        os.path.splitext(str(image_part.partname))[1].lower() or ".img",
                                    )
                                    image_filename = f"image_{image_counter}{image_suffix}"
...
class MarkdownDocumentParser(DocumentParser):
    ...
    def _download_and_update_images(self):
        ...
        image_pattern = r'!\[([^\]]*)\]\(([^\)]+)\)'
        image_counter = 0
        remote_images_enabled = os.getenv("ENABLE_MARKDOWN_REMOTE_IMAGES", "false").lower() == "true"
        allowed_hosts = hosts_from_env("MARKDOWN_IMAGE_ALLOWED_HOSTS")
        ...
        def replace_remote_image(match):
            ...
            if not image_url.startswith(("http://", "https://")):
                ...
                decoded_path = urllib.parse.unquote(parsed_image.path)
                local_path = Path(decoded_path.replace("/", os.sep))
                if (
                    parsed_image.scheme
                    ...
                    logger.warning("忽略不安全的 Markdown 图片路径")
                    return f"[不安全图片已忽略: {alt_text or 'image'}]"
                return match.group(0)
            if not remote_images_enabled:
                logger.warning("忽略未启用的 Markdown 远程图片: {}", image_url)
                return f"[远程图片已忽略: {alt_text or 'image'}]"

            try:
                validate_http_url(image_url, allowed_hosts=allowed_hosts)
                image_counter += 1
                ...
                download_utils.download_file(
                    ...
                    max_bytes=int(os.getenv("MAX_REMOTE_IMAGE_SIZE", 10 * 1024 * 1024)),
                )
                return f"![{alt_text}](images/{image_filename})"
            except Exception as exc:
                logger.warning("Markdown 远程图片处理失败: {} ({})", image_url, type(exc).__name__)
                return f"[远程图片处理失败: {alt_text or 'image'}]"

        updated_content = re.sub(image_pattern, replace_remote_image, content)
...
class PdfParser(DocumentParser):
    ...
    def _get_pdf_page_count(self) -> int:
        """获取PDF文件的总页数"""
        try:
            with pymupdf.open(self._file_path) as doc:
                    ...
                    for start_page in range(0, total_pages, CHUNK_SIZE):
                        ...
                        if self._split_pdf_by_pages(start_page, end_page, chunk_pdf_path):
                            ...
                            if chunk_content:
                                ...
                                # 对该分块使用直接提取方法
                                try:
                                    with pymupdf.open(chunk_pdf_path) as chunk_doc:
                                        chunk_text = ""
    ...
    def parse(self):
        ...
        try:
            ...
            with pymupdf.open(self._file_path) as document:
                for i, page in enumerate(document, start=1):
                    page_filename = f"page_{i}.png"
                    page_path = os.path.join(self._pages_dir, page_filename)
                    page.get_pixmap(matrix=pymupdf.Matrix(1.5, 1.5), alpha=False).save(page_path)
                    logger.debug(f"保存页面: {page_filename}")

                logger.info(f"PDF页面预览生成完成，共 {document.page_count} 页")

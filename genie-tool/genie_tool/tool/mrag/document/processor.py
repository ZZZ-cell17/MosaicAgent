...
class DocumentProcessor:
    ...
    def pre_process(self):
        ...
        if self._parser.parse() is False:
            raise RuntimeError(f"文档解析失败: {self._filename}")
    ...
    def _process_text(self):
        ...
        logger.debug("Split {} into {} text chunks", self._filename, len(chunk_texts))
        # 增加chunk的meta信息和
        batch_size = 10
        for i in tqdm.tqdm(range(0, len(chunk_texts), batch_size), desc="Processing text",
            ...
            if not self._vector_store.add_text_chunks(chunk_data):
                raise RuntimeError("文本向量写入失败")
            logger.info(f"Processed {len(chunk_texts_batch)} text chunks")

        enable_sub_chunk = os.getenv("ENABLE_SUB_CHUNK_ENHANCE", "true").lower() == "true"
        ...
        if enable_sub_chunk:
            ...
            for i in tqdm.tqdm(range(0, len(sub_texts), batch_size), desc="Processing sub text",
                ...
                if not self._vector_store.add_text_chunks(sub_chunk_data):
                    raise RuntimeError("子块向量写入失败")
    ...
    def _process_image(self):
        ...
        for i in tqdm.tqdm(range(0, len(image_paths), batch_size), desc="Process image",
            ...
            if not self._vector_store.add_image_chunks(chunk_data):
                raise RuntimeError("图片向量写入失败")
            ...
            if text_chunk_data and not self._vector_store.add_text_chunks(text_chunk_data):
                raise RuntimeError("图片 OCR/描述向量写入失败")
    ...
    def _process_page(self):
        ...
        for i in tqdm.tqdm(range(0, len(page_paths), batch_size), desc="Process page",
            ...
            if not self._vector_store.add_page_chunks(chunk_data):
                raise RuntimeError("页面向量写入失败")
        ...
        if text_chunk_data and not self._vector_store.add_text_chunks(text_chunk_data):
            raise RuntimeError("页面 OCR/描述向量写入失败")

...
class QwenVLEmbedding(ImageEmbedding):
    ...
    def _encode_image(self, image: Image.Image) -> list[float]:
        ...
        return self._extract_embedding(resp, "图片")

    @staticmethod
    def _extract_embedding(resp: requests.Response, modality: str) -> list[float]:
        if resp.status_code != HTTPStatus.OK:
            logger.error("{}编码失败: HTTP {}", modality, resp.status_code)
            raise RuntimeError(f"{modality}编码失败: HTTP {resp.status_code}")
        try:
            embedding = resp.json()["output"]["embeddings"][0]["embedding"]
        except (KeyError, IndexError, TypeError, ValueError) as exc:
            raise RuntimeError(f"{modality}编码响应格式无效") from exc
        if not embedding:
            raise RuntimeError(f"{modality}编码返回空向量")
        return embedding
    ...
    def _encode_text(self, text: str):
        try:
            ...
            return self._extract_embedding(resp, "文本")


        except Exception as e:
            logger.exception("文本编码失败")
            raise RuntimeError(f"文本编码失败: {e}") from e
...
def get_image_embedding_model() -> ImageEmbedding:
    """获取图像embedding模型"""
    image_embedding_type = os.getenv("IMAGE_EMBEDDING_TYPE", "dashscope")

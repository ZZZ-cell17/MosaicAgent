...
"""
    ...
    NO_EVIDENCE_PROMPT = """你是一个严谨的知识库问答助手。
用户的问题需要查询知识库，但当前没有检索到足以支撑答案的资料。
请直接说明暂时无法从当前知识库得到答案，并建议用户补充资料或调整问题。
不要使用模型自身知识编造答案，也不要声称执行了不存在的操作。

用户问题：{question}
输出："""

    GENERATION_ROUTE_PROMPT = """你是多模态问答路由器，需要在文本模型 llm 和视觉模型 vlm 之间选择一个。
只有回答必须查看图片、图表、版式、页面结构或其他视觉细节时才选择 vlm；如果已有文本证据足以回答则选择 llm。
只输出一个 JSON 对象，不要输出分析过程：
{{"route": "llm", "reason": "简短的路由依据"}}

用户问题：{question}
已检索到的视觉证据类型：{visual_types}
输出："""
...
# 评估步骤（请在内部完成，不要展示分析过程）
你必须按顺序完成以下三个步骤，并仅输出最终的结构化判断：
步骤一：判断查询类型
...
`reason`: 简短的决策依据，不超过100字，不得输出思维过程

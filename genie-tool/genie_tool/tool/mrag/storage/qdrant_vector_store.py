...
class QdrantVectorStore(BaseVectorStore):
    ...
    def _create_client(self) -> QdrantClient:
        ...
        if mode == "local":
            local_path = Path(os.getenv("QDRANT_PATH", "./data/qdrant")).expanduser().resolve()
            local_path.parent.mkdir(parents=True, exist_ok=True)
            logger.info(f"Creating local Qdrant client at: {local_path}")
            return QdrantClient(path=str(local_path))

        if mode != "server":
            raise ValueError("QDRANT_MODE must be 'local' or 'server'")
    ...
    def search_vectors(self,
        ...
        try:
            ...
            logger.debug(
                "Search collection {} returned {} batches",
                collection_name,
                len(search_results),
            )
    ...
    def keyword_search(self,
        ...
        try:
            ...
            logger.debug(
                "Qdrant keyword search batch_count={}",
    ...
    def create_index(self,
                     ...
                     field_type: str) -> bool:
        """创建索引"""
        if os.getenv("QDRANT_MODE", "local").strip().lower() == "local":
            logger.debug("Skip payload index in local Qdrant mode")
            return True
...
class QdrantTextVectorStore(BaseCollectionVectorStore):
    ...
    def add_docs(self, text_chunks: List[Dict]) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to add text documents to {self.collection_name}: {e}")
            return False
